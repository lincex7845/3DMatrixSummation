...
//  Uncomment the lines below when you are running tests with maven
	...
	public void testGetCubeSummation() {
		...
		cubeSummationTests.add(new CubeSummationTestCase(4, 
				new String[]{ "UPDATE 2 2 2 4", "QUERY 1 1 1 3 3 3", 
				"UPDATE 1 1 1 23", "QUERY 2 2 2 4 4 4", "QUERY 1 1 1 3 3 3"}));
		cubeSummationTests.add(new CubeSummationTestCase(2, 
				new String[]{"UPDATE 2 2 2 1", "QUERY 1 1 1 1 1 1",
		"QUERY 1 1 1 2 2 2", "QUERY 2 2 2 2 2 2"}));
		CubeSummationRequest requestCubeSummation = new CubeSummationRequest(cubeSummationTests);
		WebResource resource = CLIENT.resource(CUBE_SUMMATION_URL);
		ClientResponse response = resource
				.type(MediaType.APPLICATION_JSON_TYPE)
				.post(ClientResponse.class, requestCubeSummation);
		int status = response.getStatus();
		assertEquals(200, status);
		String entity = response.getEntity(String.class);
		assertFalse(StringUtils.isBlank(entity));
		System.out.println(entity);
		CubeSummationResponse responseCubeSummation = GSON.fromJson(entity, CubeSummationResponse.class);
